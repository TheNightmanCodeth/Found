...
public class AvatarImageBehavior extends CoordinatorLayout.Behavior<CircleImageView> {
    ...
    public AvatarImageBehavior(Context context, AttributeSet attrs) {
        ...
        if (attrs != null) {
            ...
            mStartYPosition       = (int) a.getDimension(R.styleable.AvatarImageBehavior_startYPosition, 0);
    ...
    private void maybeInitProperties(CircleImageView child, View dependency) {
        ...
        if (mFinalXPosition == 0)
            mFinalXPosition = mContext.getResources().getDimensionPixelOffset(R.dimen.abc_action_bar_content_inset_material) + ((int)mCustomFinalHeight/2)-30;

        if (mStartToolbarPosition == 0)
            mStartToolbarPosition = dependency.getY();

        if (mChangeBehaviorPoint == 0) {
            mChangeBehaviorPoint = (child.getHeight() - mCustomFinalHeight) / (.65f * (mStartYPosition - mFinalYPosition));
        }
    }
